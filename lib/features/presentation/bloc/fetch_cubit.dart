...
class FetchNewsCubit extends Cubit<FetchNews> {
  ...
  Future<void> _loadFavorites() async{
    final currentUser = FirebaseAuth.instance.currentUser; //For getting currently login firebase user.
    if (currentUser?.email == null) return; //If user is not login return null.

    final favorites = await DatabaseHelper.instance.getFavorites(); //get the list of Favorites News from the database and store in favorite variable
    favoriteArticleIds = favorites.map((f)=>f.articleId?? '').where((id)=> id.isNotEmpty).toList();
  }
  ...
  Future<void> toggleFavorite(NewsData news, BuildContext context)async{
    ...
    try{
      final isFavorite = favoriteArticleIds.contains(news.articleId);// Check articleId is favorite or not?
      if(isFavorite){
      ...
      showTopSnackBar(Overlay.of(context),
        CustomSnackBar.info(message: isFavorite? "Remove from favorites" : "Added to favorites",
          backgroundColor: isFavorite ? Colors.redAccent : Colors.blueGrey,
        )
      );
    } catch (e){
      showTopSnackBar(Overlay.of(context), CustomSnackBar.error(message: "Failed to update favorites"));
    }
  }
